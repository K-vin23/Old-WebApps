...
<body id="Main" class="Main">
  ...
  <section class="Bib-publi">
    <?php 
     if (!isset($_GET['Filmat'])) {
      $query = "SELECT * FROM material_subido WHERE IDrecursos = 3";
    }
    if (isset($_GET['Filmat'])){
    $filtro = $_GET['Filmat'];
      $query = "SELECT * FROM material_subido WHERE IDrecursos = 3 AND IDmateria = '$filtro'";
    }
