...
include 'Connect.php';
session_start();
  $sql = "SELECT IDrecursos FROM recursos";
  $ini = mysqli_query($Connect, $sql);
  $recurso = mysqli_fetch_array($ini);
  $_SESSION['Recurso']= $recurso[0];
 ?>
<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
	<title>Learn Easy - Podcast</title>
...
<body id="Main" class="Main">
  ...
  <section class="Bib-publi">
    <?php 
    if (!isset($_GET['Filmat'])) {
      $query = "SELECT * FROM material_subido WHERE IDrecursos = 1";
    }
    if (isset($_GET['Filmat'])){
    $filtro = $_GET['Filmat'];
      $query = "SELECT * FROM material_subido WHERE IDrecursos = 1 AND IDmateria = '$filtro'";
    }
    ...
    <!--Publicacion C/U -->
     ...
     ?>
     <button><a href="Publicacion.php?IDmat=<?php echo $p[0]; ?>">Abrir Archivo</a></button>
    </section>
  <?php } ?>
  </section>
  <?php
